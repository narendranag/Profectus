 ...
 * @version 	1.6.2 ($Rev: 294 $) (Based on DataMapper 1.6.0)
 */
...
class DataMapper implements IteratorAggregate {
	 ...
	 *  
	 * @param object $limit [optional] Query limit
	 * @param object $offset [optional] Query offset
	 * @return	String 
	 */
	function get_sql($limit = NULL, $offset = NULL)
	{
	 ...
	 * 3) A field on this model: "@property"  (Also, "@<whatever>" will be copied directly
	 * 4) A field on a related or deeply related model: "@model/property" "@model/other_model/property"
	 * 5) An array, which is processed recursively as a function.
	 * 
	 * @param object $function_name function name
	 * @param mixed ... Any commands that need to be passed to the function
	 * @return 
	 */
	...
	function _process_function_arg($arg, $is_formula = FALSE)
	{
		$ret = '';
		if(is_array($arg)) {
			// formula
	 ...
	 * @return 
	 */
	function _func($query, $args)
	{
		if($query == 'select')
		{
			if(count($args) < 2)
			{
				throw new Exception('Invalid number of arguments to select_func: must be at least 2 arguments.');
			}
			$alias = array_pop($args);
			$value = call_user_func_array(array($this, 'func'), $args);
			// we don't want to override the global protect_identifiers
			$protect = $this->db->_protect_identifiers;
			$ret = $this->select("$value AS $alias", FALSE);
			$this->db->_protect_identifiers = $protect;
			return $ret;
		}
		else
		{
	...
	function _subquery($query, $args)
	{
		...
		if($query == 'select')
		{
			...
			// we don't want to override the global protect_identifiers
			$protect = $this->db->_protect_identifiers;
			$ret = $this->select("$sql AS $alias", FALSE);
			$this->db->_protect_identifiers = $protect;
			return $ret;
		}
		else
		{
			...
			return $this->{$query}($field, $value, FALSE);
		}
	}
	 ...
	 * @return 
	 */
	function _parse_subquery_object($sql)
	{
		if(is_object($sql))
		{
			$sql = '(' . $sql->get_sql() . ')';
		...
		$replacement = $this->db->_escape_identifiers($this->table . '_subquery') . '.$1';
		$sql = preg_replace($pattern, $replacement, $sql);
		
		// now replace all AS table references
		// important: the space at the end is required
		$pattern = '/ AS ' . $table_pattern . ' /i';
		$replacement = ' AS ' . $this->db->_escape_identifiers($this->table . '_subquery') . ' ';
		$sql = preg_replace($pattern, $replacement, $sql);
	 ...
	 * @param object $db [optional] Private, do not use.
	 ...
	 */
	function _add_related_table($object, $related_field = '', $db = NULL, &$query_related = NULL, $name_prepend = '', $this_table = NULL)
	{
		if ( is_string($object))
		{
		...
		if(strpos($related_field, '/') !== FALSE)
		{
			...
			foreach($rfs as $rf)
			{
				$object_as = $last->_add_related_table($rf, '', $this->db, $this->query_related, $prepend, $object_as);
				$prepend .= $rf . '_'; 
				$last = $last->{$rf};
			}
			return $object_as;
		...
		if ($relationship_table == $this->table && in_array($other_column, $this->fields))
		{
			...
			{
				$db->join($object->table . ' as ' .$object_as, $object_as . '.id = ' . $this_table . '.' . $other_column, 'LEFT OUTER');
		...
		else if ($relationship_table == $object->table && in_array($this_column, $object->fields))
		{
			...
			if ( ! in_array($object_as, $query_related))
			{
				$db->join($object->table . ' as ' .$object_as, $this_table . '.id = ' . $object_as . '.' . $this_column, 'LEFT OUTER');
				$query_related[] = $object_as;
			}
			$other_column = NULL;
		}
		else
		{
			...
			{
				$db->join($relationship_table . ' as ' . $relationship_as, $this_table . '.id = ' . $relationship_as . '.' . $this_column, 'LEFT OUTER');
			...
			if ( ! in_array($object_as, $query_related))
			{
				$db->join($object->table . ' as ' . $object_as, $object_as . '.id = ' . $relationship_as . '.' . $other_column, 'LEFT OUTER');
	 ...
	 * @return	object
	 */
	function _related($query, $arguments = array(), $extra = NULL)
	{
		if ( ! empty($query) && ! empty($arguments))
		{
			...
			{
				$this->{$query}($object_table . '.' . $field, $value, $extra);
			}
		}
	 ...
	 * @return 
	 */
	function _related_subquery($query, $args)
	{
		$rel_object = $args[0];
		$field = $value = NULL;
		if(isset($args[2])) {
	 ...
	 * @param array $fields[optional] The fields to join (NULL or '*' means all fields, or use a single field or array of fields)
	 * @param bool $append_name[optional] The name to use for joining (with '_'), or FALSE to disable.
	 * @param bool $instantiate[optional] If TRUE, the results are instantiated into objects
	 */
	function include_related($related_field, $fields = NULL, $append_name = TRUE, $instantiate = FALSE)
	{
		...
		else
		{
			if($instantiate)
			{
				if(is_null($this->instantiations))
				{
					$this->instantiations = array();
				}
				$this->instantiations[$related_field] = $property_map;
			}
	 ...
	 * 
	 * @param object $related_field Field to count
	 * @param object $alias [optional] Alternative alias.
	 * @return 
	 */
	function include_related_count($related_field, $alias = NULL)
	{
		if (is_object($related_field))
		{
			$object = $related_field;
	...
	function _to_object($item, $row)
	{
		...
		if($this->instantiations) {
			foreach($this->instantiations as $related_field => $field_map) {
